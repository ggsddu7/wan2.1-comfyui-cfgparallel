...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, freqs):
        ...
        if True:
            ...
            # import pudb; pu.db
            shard_hc, seqlen = n // 2, 2 * s
            q = (q.reshape(b, s, 2, shard_hc, d).transpose(0, 2).contiguous())
            a2aout = torch.empty_like(q)
            dist.all_to_all_single(a2aout, q)
            q = a2aout.reshape(seqlen, b, shard_hc, d)
            # print(f"WanSelfAttention-22 {rank} {q.shape} {a2aout.shape}")
            q = q.transpose(0,1).contiguous().reshape(b, seqlen, shard_hc, d)
            # print(f"WanSelfAttention-33 {rank} {q.shape} {a2aout.shape}")
            a2aout.zero_()

            k = (k.reshape(b, s, 2, shard_hc, d).transpose(0, 2).contiguous())
            dist.all_to_all_single(a2aout, k)
            k = a2aout.reshape(seqlen, b, shard_hc, d)
            k = k.transpose(0,1).contiguous().reshape(b, seqlen, shard_hc, d)
            a2aout.zero_()

            v = (v.reshape(b, s, 2, shard_hc, d).transpose(0, 2).contiguous())
            dist.all_to_all_single(a2aout, v)
            v = a2aout.reshape(seqlen, b, shard_hc, d)
            v = v.transpose(0,1).contiguous().reshape(b, seqlen, shard_hc, d)
            a2aout.zero_()

            x = optimized_attention(
                q.view(b, seqlen, shard_hc * d),
                k.view(b, seqlen, shard_hc * d),
                v.view(b, seqlen, shard_hc * d),
                heads=shard_hc,
            )
            x = x.reshape(b, seqlen, shard_hc, d)
            # print(f"WanSelfAttention-55 {rank} {x.shape}")
            x = (x.reshape(b, 2, s, shard_hc, d).transpose(0, 3).transpose(0, 1).contiguous().reshape(2, shard_hc, s, b, d))
            # print(f"WanSelfAttention-66 {rank} {x.shape}")
            a2aout = torch.empty_like(x)
            dist.all_to_all_single(a2aout, x)
            # print(f"WanSelfAttention-66 {rank} {x.shape}")
            a2aout = a2aout.reshape(n, s, b, d)
            # print(f"WanSelfAttention-77 {rank} {x.shape}")
            a2aout = a2aout.transpose(0, 2).contiguous().reshape(b, s, n, d)
            x = a2aout.reshape(b, s, (n*d))
            # print(f"WanSelfAttention-88 {rank} {x.shape}")
